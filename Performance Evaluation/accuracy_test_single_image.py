...
int_to_name = {0: "ben_afflek", 1: "elton_john",
               2: "jerry_seinfeld", 3: "madonna", 4: "mindy_kaling", 5: "Nabil"}
threshold = 12
...
#------------------------------------------------------------#
#----------------------for single picture--------------#
faces = os.listdir("test/single_pic")
b_path = 'test/'
final_embeddings = []
labels = []
i = 0
for face in faces:
    image = cv2.imread(b_path+face)
...
for pic in pics:
    ...
    boxes = f.detect_face_dnn(net, image, 0.5)
    # boxes = f.detect_face_haar_cascade(cascade_path, image, 1.3, 5)  # Haar cascade
    ...
    if len(boxes) >= 1 and not check_tuple:
        ...
        cv2.imwrite("test/hudai/"+pic, image)
        # time.sleep(2)
    elif len(boxes) == 0:
